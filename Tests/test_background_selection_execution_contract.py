...
def contract_test_file(repo):
    return repo / "Tests" / "test_background_selection_execution_contract.py"


def refresh_audited_digest(repo, key, path):
    checker = repo / "scripts" / "check-swift-samples.py"
    digest = hashlib.sha256(path.read_bytes()).hexdigest()
    text = checker.read_text(encoding="utf-8")
    pattern = re.compile(rf'({re.escape(key)}:\s*")[0-9a-f]{{64}}(")')
...
def mutate_project(repo, old, new):
    project = project_file(repo)
    replace_once(project, old, new)
    refresh_audited_digest(repo, "BACKGROUND_SWITCHER_PROJECT", project)


def exact_expected_test_names(repo):
    runner = (repo / "scripts" / "run-contract-tests.py").read_text(encoding="utf-8")
    return sorted(set(re.findall(r'"(BackgroundSelectionExecutionContractTests\.test_[^"]+)"', runner)))
...
class BackgroundSelectionExecutionContractTests(unittest.TestCase):
    def test_lint_requires_automatic_xcode_generated_swift_sources_disabled(self):
        with DisposableRepository() as repo:
            refresh_audited_digest(repo, "CONTRACT_TEST_SOURCE", contract_test_file(repo))
            project = project_file(repo)
            setting = "ASSETCATALOG_COMPILER_GENERATE_ASSET_SYMBOLS = NO;\n"
            project.write_text(project.read_text(encoding="utf-8").replace(setting, ""), encoding="utf-8")
            refresh_audited_digest(repo, "BACKGROUND_SWITCHER_PROJECT", project)

            assert_lint_rejects(
                self,
                repo,
                "lint accepted automatic Xcode-generated Swift asset symbols",
            )

    def test_lint_rejects_every_executable_xcode_graph_route(self):
        mutations = (
            (
                "pre-sources shell script",
                lambda repo: mutate_project(
                    repo,
                    "\t\t\tbuildPhases = (\n\t\t\t\tFD4BC14F193F84C100102D5D /* Sources */,",
                    "\t\t\tbuildPhases = (\n"
                    "\t\t\t\tF00DBA5E0000000000000010 /* Hidden Script */,\n"
                    "\t\t\t\tFD4BC14F193F84C100102D5D /* Sources */,",
                )
                or mutate_project(
                    repo,
                    ...
                    "\t\tF00DBA5E0000000000000010 /* Hidden Script */ = {isa = PBXShellScriptBuildPhase; "
                    "buildActionMask = 2147483647; files = (); inputPaths = (); outputPaths = (); "
                    "runOnlyForDeploymentPostprocessing = 0; shellPath = /bin/sh; "
                    "shellScript = \"touch /tmp/hidden-phase\"; };\n"
            ...
            (
                ...
                lambda repo: mutate_project(
                    ...
                    "\t\t\tbuildPhases = (\n\t\t\t\tFD4BC14F193F84C100102D5D /* Sources */,",
                    "\t\t\tbuildPhases = (\n"
                    "\t\t\t\tF00DBA5E0000000000000011 /* Copy Files */,\n"
                    "\t\t\t\tFD4BC14F193F84C100102D5D /* Sources */,",
                )
                or mutate_project(
                    repo,
                    ...
                    "\t\tF00DBA5E0000000000000011 /* Copy Files */ = {isa = PBXCopyFilesBuildPhase; "
                    "buildActionMask = 2147483647; dstPath = \"\"; dstSubfolderSpec = 16; files = (); "
            ...
            (
                ...
                lambda repo: mutate_project(
                    repo,
                    "\t\t\tbuildRules = (\n\t\t\t);",
                    "\t\t\tbuildRules = (\n\t\t\t\tF00DBA5E0000000000000012 /* Hidden Rule */,\n\t\t\t);",
                )
                or mutate_project(
                    repo,
                    "/* Begin PBXBuildFile section */",
                    "/* Begin PBXBuildRule section */\n"
                    "\t\tF00DBA5E0000000000000012 /* Hidden Rule */ = {isa = PBXBuildRule; "
                    "compilerSpec = com.apple.compilers.proxy.script; filePatterns = \"*.swift\"; "
                    "fileType = pattern.proxy; isEditable = 1; outputFiles = (); script = \"touch /tmp/rule\"; };\n"
            ...
            (
                ...
                lambda repo: mutate_project(
                    repo,
                    "\t\t\t\tFD4BC14F193F84C100102D5D /* Sources */,\n"
                    "\t\t\t\tFD4BC150193F84C100102D5D /* Frameworks */,",
                    "\t\t\t\tFD4BC150193F84C100102D5D /* Frameworks */,\n"
                    "\t\t\t\tFD4BC14F193F84C100102D5D /* Sources */,",
                ),
            ),
            (
                ...
                lambda repo: mutate_project(
                    ...
                    "\t\tF00DBA5E0000000000000013 /* Hidden Aggregate */ = {isa = PBXAggregateTarget; "
                    "buildConfigurationList = FD4BC16F193F84C100102D5D; buildPhases = (); buildRules = (); "
                    "dependencies = (); name = HiddenAggregate; productName = HiddenAggregate; };\n"
                    "/* End PBXAggregateTarget section */\n\n"
                    "/* Begin PBXNativeTarget section */",
                )
                or mutate_project(
                    repo,
                    "\t\t\t\tFD4BC164193F84C100102D5D /* background_switcherTests */,\n\t\t\t);",
                    "\t\t\t\tFD4BC164193F84C100102D5D /* background_switcherTests */,\n"
                    "\t\t\t\tF00DBA5E0000000000000013 /* Hidden Aggregate */,\n\t\t\t);",
                ),
            ),
            (
                "external xcconfig",
                lambda repo: mutate_project(
                    repo,
                    "\t\tFD4BC16D193F84C100102D5D /* Debug */ = {\n\t\t\tisa = XCBuildConfiguration;",
                    "\t\tFD4BC16D193F84C100102D5D /* Debug */ = {\n"
                    "\t\t\tisa = XCBuildConfiguration;\n"
                    "\t\t\tbaseConfigurationReference = F00DBA5E0000000000000014;",
                ),
            ),
            (
                "Swift package plugin reference",
                lambda repo: mutate_project(
                    repo,
                    "/* Begin XCBuildConfiguration section */",
                    "/* Begin XCRemoteSwiftPackageReference section */\n"
                    "\t\tF00DBA5E0000000000000015 /* XCRemoteSwiftPackageReference */ = {"
                    "isa = XCRemoteSwiftPackageReference; repositoryURL = \"https://example.invalid/plugin\"; "
                ...
                or mutate_project(
                    ...
                    "\t\t\tprojectRoot = \"\";",
                    "\t\t\tprojectRoot = \"\";\n"
                    "\t\t\tpackageReferences = (F00DBA5E0000000000000015,);",
                ),
            ),
            (
                "generated source injection",
                lambda repo: add_swift_file_reference(
                    repo,
                    "Injected.swift",
                    path='"$(DERIVED_FILE_DIR)/Injected.swift"',
        ...
        for name, mutate in mutations:
            with self.subTest(name=name), DisposableRepository() as repo:
                refresh_audited_digest(repo, "CONTRACT_TEST_SOURCE", contract_test_file(repo))
                mutate(repo)
                assert_lint_rejects(self, repo, f"lint accepted {name}")
        ...
        with DisposableRepository() as repo:
            ...
            scheme = (
                repo
                / "background_switcher"
                / "background_switcher.xcodeproj"
                / "xcshareddata"
                / "xcschemes"
                / "background_switcher.xcscheme"
            )
            scheme.parent.mkdir(parents=True)
            scheme.write_text(
                '<Scheme><BuildAction><PreActions><ExecutionAction ActionType="Xcode.IDEStandardExecutionActionsCore.ExecutionActionType.ShellScriptAction"/></PreActions></BuildAction></Scheme>\n',
                encoding="utf-8",
            )
            assert_lint_rejects(self, repo, "lint accepted an executable scheme pre-action")

    def test_contract_runner_rejects_exact_name_noop_suite_after_digest_rotation(self):
        with DisposableRepository() as repo:
            names = exact_expected_test_names(repo)
            methods = "\n".join(
                f"    def {name.rsplit('.', 1)[1]}(self):\n        pass" for name in names
            )
            tests = contract_test_file(repo)
            tests.write_text(
                "import unittest\n\n"
                "class BackgroundSelectionExecutionContractTests(unittest.TestCase):\n"
                f"{methods}\n",
            ...
            refresh_audited_digest(repo, "CONTRACT_TEST_SOURCE", tests)

            result = run([MAKE, "contract-test"], repo)

            self.assertNotEqual(
                result.returncode,
                0,
                "contract-test accepted exact-name no-op laundering after coordinated digest rotation",
            )
    ...
    def test_lint_rejects_delayed_uikit_backed_mapping_corruption(self):
        with DisposableRepository() as repo:
            controller = controller_file(repo)
            replace(
                controller,
                ...
                textwrap.dedent(
                    """
                            imageView.accessibilityIdentifier = (imageView.accessibilityIdentifier ?? "") + "x"
                            if imageView.accessibilityIdentifier!.count > 4096 {
                                self.backgroundDict["Background1"] = .black
                            }
                            if let backgroundColor = self.backgroundDict[selection.key] {
                    """
                ).rstrip(),
            ...
            assert_lint_rejects(
                self,
                repo,
                "lint accepted delayed mapping corruption hidden in approved UIKit object state",
            )

    def test_contract_runner_rejects_class_level_skip_laundering(self):
        with DisposableRepository() as repo:
            tests = contract_test_file(repo)
            tests.write_text(
                textwrap.dedent(
            ...
            result = run([sys.executable, repo / "scripts" / "run-contract-tests.py"], repo)

            self.assertNotEqual(
                result.returncode,
                0,
                "contract-test accepted a completely skipped adversarial suite",
            )

    def test_contract_runner_rejects_missing_identity_skips_and_expected_failures(self):
        mutations = (
            (
        ...
        for name, suite in mutations:
            with self.subTest(name=name), DisposableRepository() as repo:
                contract_test_file(repo).write_text(
                    "import unittest\n\n"
                    + textwrap.dedent(suite).lstrip()
                    + '\n\nif __name__ == "__main__":\n    unittest.main()\n',
                    encoding="utf-8",
                )
                ...
                self.assertNotEqual(
                    result.returncode,
                    0,
                    f"contract-test accepted {name} laundering",
                )

    def test_lint_rejects_app_delegate_application_overload_in_extension(self):
        with DisposableRepository() as repo:
            app_delegate = app_delegate_file(repo)
            app_delegate.write_text(
                app_delegate.read_text(encoding="utf-8")
                + textwrap.dedent(
                    """
            ...
            assert_lint_rejects(
                self,
                repo,
                "lint accepted an arbitrary UIApplicationDelegate overload in an AppDelegate extension",
            )

    def test_lint_rejects_app_delegate_application_overload_in_alternate_project_source(self):
        with DisposableRepository() as repo:
            add_source_file(
                repo,
            ...
            add_swift_file_reference(repo, "AppDelegateExtension.swift")

            assert_lint_rejects(
                self,
                repo,
                "lint accepted an arbitrary UIApplicationDelegate overload in an alternate compiled source",
            )
