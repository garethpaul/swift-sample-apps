...
def require_fetch_source(url, repository_slug, label, allow_local_fetch):
    if not isinstance(url, str) or not url:
        raise VerificationError(f"{label} fetch URL is required")
    if not REPOSITORY_PATTERN.fullmatch(repository_slug or ""):
        raise VerificationError(f"{label} repository slug is invalid")

    parsed = urllib.parse.urlparse(url)
    if allow_local_fetch and parsed.scheme == "file" and parsed.netloc in {"", "localhost"}:
        path = Path(urllib.parse.unquote(parsed.path))
        if path.is_absolute():
            return {"allow_file": True, "url": url}

    expected_path = f"/{repository_slug}.git"
    if (
        parsed.scheme != "https"
        ...
        raise VerificationError(f"{label} fetch URL must be https://github.com/{repository_slug}.git")
    return {"allow_file": False, "url": url}


def fetch_protocol_options(source):
    file_policy = "always" if source["allow_file"] else "never"
    return [
        "-c",
        "protocol.version=2",
        "-c",
...
def ensure_candidate_history(candidate_repository, head_sha, candidate_source):
    shallow = git_text(candidate_repository, "rev-parse", "--is-shallow-repository")
    if shallow == "false":
        return
    if shallow != "true":
        raise VerificationError("unable to determine whether candidate checkout is shallow")
    deepen = run_git(
        candidate_repository,
        *fetch_protocol_options(candidate_source),
        "fetch",
        "--no-tags",
        "--deepen=64",
    ...
    if deepen.returncode != 0:
        raise VerificationError("unable to deepen candidate history for rejected ancestry proof")
    if git_text(candidate_repository, "rev-parse", "--is-shallow-repository") == "true":
        raise VerificationError("candidate history remains shallow after bounded fetch")
...
def ensure_rejected_commit(candidate_repository, trusted_source, rejected):
    exists = run_git(candidate_repository, "cat-file", "-e", f"{rejected}^{{commit}}", check=False)
    if exists.returncode == 0:
        return
    imported = run_git(
        candidate_repository,
        *fetch_protocol_options(trusted_source),
        "fetch",
        "--no-tags",
        "--depth=64",
        trusted_source["url"],
    ...
    if imported.returncode != 0:
        raise VerificationError(f"rejected candidate object is unavailable: {rejected}")
    exists = run_git(candidate_repository, "cat-file", "-e", f"{rejected}^{{commit}}", check=False)
    if exists.returncode != 0:
        raise VerificationError(f"rejected candidate object is unavailable: {rejected}")


def verify_topology(candidate_repository, trusted_source, candidate_source, base_sha, head_sha, rejected_commits):
    ...
    ensure_candidate_history(candidate_repository, head_sha, candidate_source)
    ...
    for rejected in rejected_commits:
        ...
        ensure_rejected_commit(candidate_repository, trusted_source, rejected)
...
def verify(arguments):
    ...
    if not isinstance(rejected, list) or not rejected:
        raise VerificationError("trusted policy must retain rejected candidates")
    trusted_source = require_fetch_source(
        arguments.trusted_fetch_url,
        arguments.trusted_repository_slug,
        "trusted",
        arguments.allow_local_fetch,
    )
    candidate_source = require_fetch_source(
        arguments.candidate_fetch_url,
        arguments.candidate_repository_slug,
        "candidate",
        arguments.allow_local_fetch,
    )
    verify_topology(candidate_repository, trusted_source, candidate_source, base_sha, head_sha, rejected)
...
def parse_arguments():
    ...
    parser.add_argument("--trusted-fetch-url", required=True)
    parser.add_argument("--candidate-fetch-url", required=True)
    parser.add_argument("--trusted-repository-slug", required=True)
    parser.add_argument("--candidate-repository-slug", required=True)
