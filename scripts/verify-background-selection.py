...
def candidate_matches(binary, sequences):
    ...
    for sequence in sequences:
        ...
        if result.returncode != 0 or result.stderr or actual != expected:
            return False, (
                f"sequence={sequence!r} status={result.returncode} "
                f"expected={expected!r} actual={actual!r} stderr={result.stderr!r}"
            )
