...
jobs:
  ...
  build:
    ...
    steps:
      ...
      - name: Run fail-closed verification before Xcode
        run: make check
      - name: Run Release XCTest through preflight
        run: make native-test-release
      - name: Build Debug configuration through preflight
        run: make build-debug
      - name: Build Release configuration through preflight
        run: make build-release
      - name: Analyze Debug configuration through preflight
        run: make analyze-debug
      - name: Analyze Release configuration through preflight
        run: make analyze-release
"""
SAMPLES = (
    "background_switcher",
    "basic-note-taker",
...
TRUSTED_CANDIDATE_VERIFIER = ROOT / "scripts/verify-trusted-candidate.py"
V11_TRUST_TEST = ROOT / "Tests/test_v11_trust_contract.py"
...
AUDITED_FILE_SHA256 = {
    ...
    CONTRACT_TEST_RUNNER: "1c3a5ee1047cb06fb95aed13ead32cc9dbcf4b632a36fce377cb75e573717d40",
    TRUSTED_CANDIDATE_VERIFIER: "09e7e9b3697e558985274862e3930ed8ab1cd5a3803077e5ad6a1114bab720d2",
    V11_TRUST_TEST: "5feefaeb382a6e0aea827a23fb93ca94e27131b772703ecaffe2a60807925533",
...
def hygiene_checks():
    ...
    for contract in (
        ...
        '"$(ROOT)/scripts/verify-trusted-candidate.py" --mode all',
