...
BACKGROUND_EXECUTION_CONTRACT_DIGESTS = {
    "Makefile": "2f6f96439dfd21bdc025166cd011d69916ac2ea4d61b9e86da486bfcdb82d5f9",
    "Tests/BackgroundSelectionTests/main.swift": "4d27f28bbac89f52562202f851fedc8774f33d44dbe48babff35fca17e52bafc",
    "Tests/test_background_selection_execution_contract.py": "1553fa94925d2d526ccd482c642eaa9a22b67f16e417b2f6ba7ff981f612f746",
    "scripts/test-background-selection.sh": "c56a57ce66a8967e17eb127198f43c0819ded0e0ef7acd26a3b79f0b28449b02",
    "scripts/verify-background-selection.py": "95f1a274b76661d36d3075aac01ab35044f1447ddfdbc3dbaa21e8f0f0178e13",
}
...
def hygiene_checks():
    ...
    for contract in (
        ...
        "override TRUSTED_DEVELOPER_DIR :=",
        "override XCODEBUILD :=",
    ):
        if makefile.count(contract) != 1:
            errors.append(f"Makefile must resolve trusted tools exactly once: {contract}")
    for contract in ("trusted Python 3 is unavailable", "trusted xcodebuild unavailable"):
        if contract not in makefile:
            errors.append(f"Makefile must fail closed when a trusted tool is absent: {contract}")
    for contract in (
    ...
    if not BACKGROUND_SELECTION_RUNNER.exists():
        ...
        for contract in (
            ...
            "trusted Python 3 is unavailable",
