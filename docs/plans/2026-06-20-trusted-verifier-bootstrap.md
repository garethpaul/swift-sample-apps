...
The verifier confirms that its workflow, policy, launcher, templates, and verifier bytes are exact blobs from the workflow SHA. It requires the candidate head to be one commit with the trusted base as its sole parent. Before checking rejected bootstrap ancestry, it deepens a shallow candidate checkout through the exact event-bound candidate source and imports each rejected SHA from the exact event-bound trusted source with bounded fetches. A rejected SHA that remains unavailable, a checkout that remains shallow, or a source URL that does not match the event repository slug fails closed. It reads commit/tree/blob objects with `/usr/bin/git --no-replace-objects`; it does not import, build, source, or execute candidate content.
